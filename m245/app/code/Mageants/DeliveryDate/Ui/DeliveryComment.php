        ...
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as &$item) {
                ...
                if (strpos($item[$this->getData('name')], '0000-00-00') !== false) {
                    ...
                    if (strpos($item[$this->getData('name')], 'delivery_comment') !== false) {
                        $deliverycomment = $this->serializer->unserialize($item[$this->getData('name')]);
                        $name = '';
                        foreach ($deliverycomment as $key => $value) {
                            $name .= $deliverycomment[$key]['delivery_comment'] . ",";
                        }
