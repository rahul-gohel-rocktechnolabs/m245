...
namespace Mageants\DeliveryDate\Controller\Cart;

use Magento\Framework\App\Action\HttpPostActionInterface as HttpPostActionInterface;
use Magento\Catalog\Api\ProductRepositoryInterface;
use Magento\Checkout\Model\Cart as CustomerCart;
use Magento\Framework\Exception\NoSuchEntityException;
use Mageants\DeliveryDate\Helper\Data;
...
class Add extends \Magento\Checkout\Controller\Cart implements HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if ($this->helper->getPluginEnable()) {
                if ($this->helper->getPluginDisplayAt() == 3) {
                    if (!isset($params['del-date']) || !isset($params['delivery_timeslot'])) {
                        $message = __('Please Select Date Delivery Information');
                        $this->messageManager->addErrorMessage($message);
                        $product = $this->productRepository->getById($params['product']);
                        return $this->goBack($product->getProductUrl(), $product);
                    }
                }
            }
